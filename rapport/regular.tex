...
of properties. \todo{wikipedia havde noget interessant at sige men vi kan ikke bare kopiere det ind :(}
...
\item For the non-capturing parenthesis we have the choice of
  ...
  be simple to implement. This
    is however the industry standard for repetition notation\todo{changed}.
  \end{itemize}
...
equivalent\todo{jan: in terms of what?}: NFAs and DFAs recognize exactly the regular languages.

\todo[inline]{jan: What about "Nondeterministic finite automata with ε-transitions (FND-ε or ε-NFA)"}
...
\item[NFA] For each pair of input symbol and state, there may be more
  ...
  through an NFA for a given\todo{jan: an?} input string.
...
\item[DFA] For each pair of input symbol and state, there may be only
  ...
  for an input string.
\end{description}
\todo[inline]{jan: Aren't you forgetting the size growth of either? Also you're not talking about the properties of either, and their respective disadvantages}
\input{parsing}
...
engine can typically solve some types of problems more efficiently than others, or vice versa: it may be particularly bad at a given problem.\todo{Jan: changed}
